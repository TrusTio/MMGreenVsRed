...
public class Grid {
	...
	public void transformGrid() {
		...
		for (int y = 0; y < heightY; y++) {
			for (int x = 0; x < widthX; x++) {
				int greenNeighbours = GridHelper.countGreenNeighbours(x, y, byteArrayCopy);
				byteArray[x][y] = GridHelper.transformCell(byteArrayCopy[x][y], greenNeighbours);
			}
		}
	}
	 ...
	 * @param x An int representing the X coordinate of the Cell.
	 * @param y An int representing the Y coordinate of the Cell.
	 * @param N An int representing the amount of generations.
	 */
